...
class IXFMemberListFromClientsCommand(ARouteServerCommand):
    ...
    @staticmethod
    def apply_merge_file(dic, merge_file, example_args_for_error):
        ...
        if "ixp_list" in merge_file:
            ...
            if not ixp_list:
                raise IXFMemberListFromClientsMergeFileError("ixp_list is empty", **example_args_for_error)
            if not isinstance(ixp_list, list):
                raise IXFMemberListFromClientsMergeFileError("ixp_list is not a list")
            if not len(ixp_list) == 1:
                raise IXFMemberListFromClientsMergeFileError("ixp_list contains more than one dictionary")
            ...
            for key in ("ixf_id", "ixp_id", "shortname"):
                if key not in ixp_list_entry:
                    raise IXFMemberListFromClientsMergeFileError(
                        "the '{}' key cannot be found in the ixp_list dictionary".format(key)
                    )

                if not ixp_list_entry[key]:
                    raise IXFMemberListFromClientsMergeFileError(
                        "the value of the '{}' key in the ixp_list dictionary is empty or not set".format(key)
                    )

            for key in ("ixf_id", "ixp_id"):
                if not isinstance(ixp_list_entry[key], int):
                    raise IXFMemberListFromClientsMergeFileError(
                        "the value of the '{}' key in the ixp_list dictionary must be an integer".format(key)
                    )
    ...
    def run(self):
        ...
        if self.args.merge_file:
            ...
            output_dic = self.apply_merge_file(dic, merge_file, example_args_for_error)
        else:
            output_dic = dic
