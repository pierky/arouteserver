...
- (BIRD and OpenBGPD) the builtin method based on `RIPE RPKI Validator format <http://localcert.ripe.net:8088/>`__ export files: the URL of a local and trusted instance of RPKI Validator should be provided to ensure that a cryptographically validated datased is used. By default, the URLs of some  public instances are used.
