...
#. `Jinja2`_ built-in templates are used to render the final route server's configuration file.
   ...
   Currently, **BIRD** (>= 1.6.3), **BIRD 2** (>= 2.0.7 - support for BIRD 2 is in early stages) and **OpenBGPD** (OpenBSD 6.1 up to 6.6 and also OpenBGPD Portable 6.5p1) are supported, with almost feature parity between them.
